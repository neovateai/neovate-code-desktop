    ...
    const hasExtensionDir = zip
      ...
      .some(
        (entry) =>
          ...
          entry.entryName.startsWith('extension/'),
      );
...
function readExtensionManifest(tempDir: string): ExtensionManifest {
  ...
  try {
    ...
    throw new Error(
      `读取扩展清单失败: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}

export async function installExtension(): Promise<void> {
  const vsixPath = getExtensionsBasePath();
  ...
  if (fs.existsSync(extensionJSON)) {
    try {
      ...
      console.error(
        '[installExtension] 读取或解析 extensions.json 失败:',
        error,
      );
      extensions = [];
    }
